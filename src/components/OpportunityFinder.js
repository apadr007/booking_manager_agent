...
const ACADEMIC_STATUSES   = ["Researched","Contacted","Confirmed","Done","Pass"];
const ACAD_PILL = { Researched:"#9B8B7A", Contacted:"#C9A87A", Confirmed:"#6DBF8A", Done:"#8B6F47", Pass:"#4A4A4A" };

// ── Default Academic data ─────────────────────────────────────
const DEFAULT_ACADEMIC = [
  { id:1, name:"CU Boulder BioFrontiers Institute Seminar", location:"Boulder, CO", date:"Ongoing 2026", type:"Seminar Series", audience:"PhD students & postdocs in biology/bioinformatics", contact:"biofrontiers.colorado.edu", link:"https://www.colorado.edu/biofrontiers", fit:"⭐⭐⭐⭐⭐ Perfect local fit", notes:"Directly reaches PhD scientists — ideal audience for scientist-to-executive coaching and AI leadership content." },
  { id:2, name:"CU Denver Business School Executive Speaker Series", location:"Denver, CO", date:"Ongoing 2026", type:"Speaker Series", audience:"MBA students and executive ed participants", contact:"business.ucdenver.edu", link:"https://business.ucdenver.edu", fit:"⭐⭐⭐⭐ Strong fit for leadership + AI content", notes:"Executive education audience actively seeking leadership development — good pipeline for coaching offer." },
  { id:3, name:"Colorado State University Life Sciences Seminar", location:"Fort Collins, CO", date:"Ongoing 2026", type:"Seminar Series", audience:"Biology, biochemistry, and pre-med PhD students", contact:"biology.colostate.edu", link:"https://www.colostate.edu", fit:"⭐⭐⭐⭐ Local reach into emerging scientist talent pool", notes:"PhD and postdoc audience — strong fit for career transition and burnout coaching topics." },
  { id:4, name:"MIT Broad Institute Career Development Talks", location:"Cambridge, MA (Virtual ok)", date:"Ongoing 2026", type:"Career Series", audience:"Life sciences PhD students and postdocs", contact:"broadinstitute.org/education", link:"https://www.broadinstitute.org", fit:"⭐⭐⭐ Prestigious — virtual option available", notes:"Top-tier life sciences talent — very receptive to scientist career transition and leadership coaching." },
  { id:5, name:"UCSF Career Development Office Speaker Series", location:"San Francisco, CA (Virtual ok)", date:"Ongoing 2026", type:"Speaker Series", audience:"Biomedical PhD students and postdocs", contact:"career.ucsf.edu", link:"https://career.ucsf.edu", fit:"⭐⭐⭐ High impact biomedical audience", notes:"UCSF students are highly receptive to non-academic career paths — ideal for coaching and AI leadership topics." },
  { id:6, name:"University of Colorado Anschutz Medical Campus", location:"Aurora, CO", date:"Ongoing 2026", type:"Grand Rounds / Seminar", audience:"Medical researchers, postdocs, MD-PhD students", contact:"ucdenver.edu/anschutz", link:"https://www.ucdenver.edu/anschutz", fit:"⭐⭐⭐⭐⭐ Top local medical research campus", notes:"Right in Alex's backyard — strong audience of scientists considering leadership or industry transition." },
];
...
async function fetchAcademic(existing) {
  const skip = existing.map(e => e.name).join(", ");
  const prompt = `Today is ${TODAY}. Find 4 universities or academic institutions actively seeking outside speakers for Alex Padron — a PhD molecular biologist turned executive coach for biotech/scientific leaders in Boulder CO. Author of "The Way of the Logos." Topics: AI leadership for scientists, scientist-to-executive career transitions, burnout and flourishing in science, leadership development for PhDs, contemplative leadership.

RULES:
1. Focus on REAL recurring programs: seminar series, career development speaker series, executive education programs, leadership institutes, grand rounds, or student org speaker programs.
2. Prioritize: life sciences / biomedical PhD programs, business schools with life sciences focus, pre-med and STEM career programs, Colorado-area universities first, then national with virtual options.
3. Include a mix of: local CO universities (CU, CSU, DU, Mines), top-tier programs open to practitioners (MIT, Stanford, UCSF, Harvard), and leadership/executive ed programs.
4. Do NOT include: ${skip}
5. For "fit" field: rate ⭐ to ⭐⭐⭐⭐⭐ on how well Alex's background and topics match this audience.

Return raw JSON only — no markdown, no code fences:
[{"name":"...","location":"City, ST","date":"Ongoing Year or Season Year","type":"Seminar Series or Speaker Series or Career Series or Executive Ed","audience":"who attends","contact":"department website or email","link":"https://...","fit":"⭐⭐⭐ one sentence","notes":"one sentence on why this fits Alex"}]`;
  const raw = await callClaude(prompt);
  return parseAIResponse(raw);
}

// ── Reusable table component ──────────────────────────────────
function OpportunityTable({ title, icon, rows, cols, statuses, pillColors, onStatusChange, onDelete, onAdd, onRefresh, isRefreshing, lastRefreshed, refreshError, emptyAdd }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:14, flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontSize:18, color:"#F5EDE0", fontWeight:"bold", letterSpacing:"0.01em" }}>{icon} {title}</div>
      ...
      {showAdd && (
        <div style={{ background:"rgba(245,237,224,0.05)", border:"1px solid rgba(201,168,122,0.2)", borderRadius:14, padding:16, marginBottom:16 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:8 }}>
            {emptyAdd.filter(f => f !== "notes").map(f => (
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={() => { onAdd({...newRow, id:Date.now(), status:"Researched"}); setNewRow({}); setShowAdd(false); }}
              style={{ flex:1, padding:"9px", background:"linear-gradient(135deg,#C9A87A,#8B6F47)", border:"none", borderRadius:10, color:"#2C1F14", cursor:"pointer", fontSize:13, fontFamily:"Georgia,serif" }}>
              Add
            </button>
            <button onClick={() => setShowAdd(false)}
              style={{ padding:"9px 18px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(201,168,122,0.18)", borderRadius:10, color:"#9B8B7A", cursor:"pointer", fontSize:13, fontFamily:"Georgia,serif" }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      <div style={{ overflowX:"auto" }}>
        ...
        {rows.length === 0 && (
          <div style={{ textAlign:"center", padding:"36px", color:"#9B8B7A", fontSize:13 }}>No entries yet — click "Find New" or "+ Add".</div>
        )}
      </div>
    </div>
  );
...
function lsSetRaw(key, val) { try { localStorage.setItem(key, val) } catch {} }
...
export default function App() {
  ...
  const [academic, setAcademic]                         = useState(DEFAULT_ACADEMIC);
  const [academicRefreshed, setAcademicRefreshed]       = useState(null);
  ...
  useEffect(() => {
    (async () => {
      const sp = lsGet('opp-speaking');     if (sp) setSpeaking(sp)
      const spt = lsGetRaw('opp-speaking-ts'); if (spt) setSpeakingRefreshed(spt)
      const nw = lsGet('opp-networking');   if (nw) setNetworking(nw)
      const nwt = lsGetRaw('opp-networking-ts'); if (nwt) setNetworkingRefreshed(nwt)
      const ac = lsGet('opp-academic');     if (ac) setAcademic(ac)
      const act = lsGetRaw('opp-academic-ts'); if (act) setAcademicRefreshed(act)
    })();
  }, []);

  // Auto-save
  useEffect(() => { (async () => { try { await window.storage.set("opp-speaking", JSON.stringify(speaking)); } catch {} })(); }, [speaking]);
  useEffect(() => { (async () => { try { await window.storage.set("opp-networking", JSON.stringify(networking)); } catch {} })(); }, [networking]);
  useEffect(() => { (async () => { try { await window.storage.set("opp-academic", JSON.stringify(academic)); } catch {} })(); }, [academic]);

  // Auto-refresh on tab open if stale
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("opp-speaking-ts");
        const last = r ? parseInt(r.value) : 0;
        if (!isNaN(last) && Date.now() - last > ONE_WEEK_MS) doRefreshSpeaking();
      } catch {}
    })();
  }, []); // on mount

  useEffect(() => {
    if (tab !== "networking") return;
    (async () => {
      try {
        const r = await window.storage.get("opp-networking-ts");
        const last = r ? parseInt(r.value) : 0;
        if (!isNaN(last) && Date.now() - last > ONE_WEEK_MS) doRefreshNetworking();
      } catch {}
    })();
  }, [tab]);

  // Auto-refresh academic when tab opened
  useEffect(()=>{
    if(tab==="academic") {
      (async()=>{
        try {
          const last = parseInt(lsGetRaw('opp-academic-ts-raw') || '0');
          if(!isNaN(last) && Date.now() - last > ONE_WEEK_MS) doRefreshAcademic();
        } catch {}
      })();
    }
  },[tab]);
  ...
  async function doRefreshSpeaking() {
    ...
    try {
      ...
      try { await window.storage.set("opp-speaking-ts", String(Date.now())); } catch {}
    } catch (e) {
      setSpeakingError("Search failed: " + (e.message || "unknown error"));
    }
  ...
  async function doRefreshNetworking() {
    ...
    try {
      ...
      try { await window.storage.set("opp-networking-ts", String(Date.now())); } catch {}
    } catch (e) {
      setNetworkingError("Search failed: " + (e.message || "unknown error"));
    }
    setNetworkingRefreshing(false);
  }

  async function doRefreshAcademic() {
    if(academicRefreshing) return;
    setAcademicRefreshing(true); setAcademicError("");
    try {
      const results = await fetchAcademic(academic);
      let added = 0;
      setAcademic(prev => {
        const merged = [...prev];
        for(const item of results) {
          const exists = merged.some(e => e.name.toLowerCase().includes(item.name.toLowerCase().slice(0,12)));
          if(!exists) { merged.push({...item, id:Date.now()+Math.random()}); added++; }
        }
        return merged;
      });
      const label = new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) + (added > 0 ? ` · +${added} new` : " · no new found");
      setAcademicRefreshed(label);
      lsSetRaw('opp-academic-ts', label); lsSetRaw('opp-academic-ts-raw', String(Date.now()))
    } catch(e) { setAcademicError("Search failed: " + (e.message||"unknown error")); }
  ...
  const academicCols = [
    { key:"name",     label:"University / Program",  w:"26%" },
    { key:"type",     label:"Type",                  w:"14%" },
    { key:"audience", label:"Audience",              w:"22%" },
    { key:"fit",      label:"Fit Rating",            w:"20%" },
    { key:"status",   label:"Status",                w:"11%" },
    { key:"actions",  label:"",                      w:"7%"  },
  ];

  const speakingAddFields  = ["name","location","date","fee","deadline","contact","link","beginner","notes"];
  const networkingAddFields = ["name","location","date","type","cost","contact","link","value","notes"];
  ...
  return (
    <div style={{
      minHeight:"100vh",
      background:"linear-gradient(160deg,#1E1610 0%,#2E1F14 35%,#3D2B1A 70%,#2A1D12 100%)",
      ...
      {/* Header */}
      <div style={{ width:"100%", maxWidth:860, marginBottom:24, animation:"fadeIn 0.4s ease-out" }}>
        <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          <div>
            <div style={{ fontSize:26, color:"#F5EDE0", letterSpacing:"0.02em", lineHeight:1.2 }}>
              ✦ Opportunity Finder
            </div>
            <div style={{ fontSize:12, color:"#9B8B7A", marginTop:5, letterSpacing:"0.04em" }}>
  {mounted ? fmtDate() : ''} · Alex Padrón · Boulder, CO
            </div>
          </div>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            <div style={{ fontSize:11, color:"#6DBF8A", background:"rgba(109,191,138,0.1)", border:"1px solid rgba(109,191,138,0.25)", borderRadius:20, padding:"4px 12px" }}>
              ● AI-powered · auto-saves
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div style={{ width:"100%", maxWidth:860, display:"flex", gap:6, marginBottom:20 }}>
        {[
          ...
          { id:"academic",   label:"🎓 Academic",    count: academic.filter(a => a.status !== "Pass" && a.status !== "Done").length },
        ].map(t => (
          <button key={t.id} className="tab-btn" onClick={() => setTab(t.id)} style={{
            padding:"10px 22px", borderRadius:14,
            background: tab===t.id ? "rgba(201,168,122,0.18)" : "rgba(255,255,255,0.03)",
            border: tab===t.id ? "1px solid rgba(201,168,122,0.45)" : "1px solid rgba(201,168,122,0.1)",
            color: tab===t.id ? "#C9A87A" : "#9B8B7A",
            fontSize:13.5, cursor:"pointer", transition:"all 0.2s", fontFamily:"Georgia,serif",
            fontWeight: tab===t.id ? "bold" : "normal",
            display:"flex", alignItems:"center", gap:8,
          }}>
            {t.label}
            <span style={{
              background: tab===t.id ? "rgba(201,168,122,0.25)" : "rgba(255,255,255,0.07)",
              borderRadius:10, padding:"1px 8px", fontSize:11, color: tab===t.id ? "#C9A87A" : "#9B8B7A"
            }}>{t.count}</span>
          </button>
        ))}
      </div>
      ...
      <div style={{
        ...
        {tab === "speaking" && (
          <OpportunityTable
            title="Speaking Opportunities" icon="🎤"
            rows={speaking} cols={speakingCols}
            statuses={SPEAKING_STATUSES} pillColors={STATUS_PILL}
            ...
            onRefresh={doRefreshSpeaking}
            isRefreshing={speakingRefreshing}
            lastRefreshed={speakingRefreshed}
            refreshError={speakingError}
            emptyAdd={speakingAddFields}
          />
        )}
        {tab === "networking" && (
          <OpportunityTable
            title="Networking Events" icon="🤝"
            rows={networking} cols={networkingCols}
            statuses={NETWORKING_STATUSES} pillColors={NET_PILL}
            ...
            onRefresh={doRefreshNetworking}
            isRefreshing={networkingRefreshing}
            lastRefreshed={networkingRefreshed}
            refreshError={networkingError}
            emptyAdd={networkingAddFields}
          />
        )}
        {tab === "academic" && (
          <OpportunityTable
            title="University & Academic Programs" icon="🎓"
            rows={academic} cols={academicCols}
            statuses={ACADEMIC_STATUSES} pillColors={ACAD_PILL}
            onStatusChange={(id, s) => setAcademic(prev => prev.map(e => e.id===id ? {...e,status:s} : e))}
            onDelete={id => setAcademic(prev => prev.filter(e => e.id!==id))}
            onAdd={item => setAcademic(prev => [...prev, item])}
            onRefresh={doRefreshAcademic}
            isRefreshing={academicRefreshing}
            lastRefreshed={academicRefreshed}
            refreshError={academicError}
            emptyAdd={["name","location","type","audience","contact","link","fit"]}
          />
        )}
      </div>

      <div style={{ marginTop:16, fontSize:11, color:"rgba(155,139,122,0.35)", textAlign:"center" }}>
        Powered by Claude · Data auto-saves · Refreshes weekly
      </div>
    </div>
  );
}
